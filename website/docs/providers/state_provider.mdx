...
//The snippet thats returned above, is that correct? Its returning the `name` property of the object `ProductSortType` so the return type for the provider at a guess would be `String` not `ProductSortType`
